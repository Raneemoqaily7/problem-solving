...
const customerAndAge = (obj) => {
 ...
 let result=[]
 for (var i = 0 ; i < customersNames.length ; i++){
   result.push(`Customer Name :${customersNames[i]} , Age :${customersAges[i]}`)
 }
 return result
};
...
const getInfo = (arr) => {
  ...
  let studentsName = [];
  arr.forEach(c => {
   coursesName.push(c.course)
   studentsName.push(...c.Students)
  })
   return { coursesName: coursesName, studentsName: studentsName };
};
