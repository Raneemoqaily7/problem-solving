...
const cvFormatter = (arr) => {
    var applicantsWithMoreThanOneYearExp = arr.filter(
      (applicant) => applicant.yearsOfExperience > 1
    );
    var filteredApplicant = [];
    applicantsWithMoreThanOneYearExp.forEach((applicant) => {
      if (applicant.lastName) {
        filteredApplicant.push({
...
const applicationsStatics = (arr) => {
    ...
    arr.forEach((application) => {
      if (application.yearsOfExperience <= 1 || application.tech=="" || application.tech == null ) {
        result.rejectedApplicants += 1;}
       if (application.tech == ".Net") {
        result.dotNet_Devs += 1;
      }
       if (application.tech == "Java") {
        result.java_Devs += 1;
      } 
       if (application.tech == "Python") {
        result.python_Devs += 1;
      } 
       if (application.tech == "JS") {
...
grade.classes.forEach(classs =>{
  classs.avg = Math.floor(arrayAvg(classs.classScores))
})
})
return data
};
